...
- ### Parking program-停車計劃
    ### 中文:
    - 當車輛行駛時，系統（Jetson Orin Nano）首先透過攝影機偵測粉紅色方塊來識別停車場的位置。當車輛到達第三圈的最後一個彎道時，它首先駛向終點區並短暫停留。然後，車輛完成另一圈，並在接近停車場附近的轉彎處時降低速度，精確停在指定位置。此時，系統使用攝影機突出顯示的數值來評估車輛與前邊界牆之間的距離，並調整其朝向停車場的方向。 
  ...
  - **Code running on the Jetson Orin Nano controller.-在 Jetson Orin Nano 控制器上執行的程式碼**
    ```
        #parking area turn
        if turn_side == 4:
            if not ROI2:
